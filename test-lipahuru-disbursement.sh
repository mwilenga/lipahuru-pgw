...
BODY=$(cat <<EOF
{"requestId":"${REQUEST_ID}","providerCode":"${PROVIDER_CODE}","amount":${AMOUNT},"currency":"TZS","msisdn":"${MSISDN}","reference":"DISB-CURL-$(date +%s)","callbackUrl":"${CALLBACK_URL}","narration":"curl disbursement test"}
EOF
)
...
if echo "$CREATE_BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); exit(0 if d.get('status')=='SUCCESS' else 1)" 2>/dev/null; then
  ...
  fail_if_not_json "Disbursement" "$CREATE_BODY"
  echo "$CREATE_BODY" | python3 -m json.tool
  exit 1
fi
